...
def se_resnet50(pretrained=False, **kwargs ):
    model = SENet(SEResNetBottleneck, [3, 4, 6, 3], groups=1, reduction=16, dropout_p=None, inplanes=64, input_3x3=False, downsample_kernel_size=1, 
                  downsample_padding=0, **kwargs)
    if pretrained is not None:                
        #model.load_state_dict(model_zoo.load_url(settings['url']))
        utl.load_state_dict(model.state_dict(), model_zoo.load_url(model_urls['se_resnet50']))
    return model
