# Based on https://www.kaggle.com/kernels/notebooks/new?forkParentScriptVersionId=7789363&userName=rapela
# download
...
file_name = 'subcellular_location.tsv'
subcellular_location = pd.read_csv(file_name, sep="\t",index_col = None)
...
for i in tqdm(range(len(subcellular_location))):
    ...
    for im in glob('external_data/'+im_name+'*'):
        labels = []
...
df = pd.DataFrame(data, columns = ['Id', 'names','Target'])
df.drop('names', axis=1, inplace=True)
df1 = df.copy(deep=True)
df['Target'] = pd.DataFrame([str(line).strip('[').strip(']').replace(",","") for line in df['Target']])
df.to_csv('train.csv', sep=',', index=False)

count_labels = Counter(list(chain.from_iterable((df1['Target'].values))))
